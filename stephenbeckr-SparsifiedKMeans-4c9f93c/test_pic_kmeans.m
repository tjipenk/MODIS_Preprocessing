...
img = imread('2016-12-24-172221.jpg');[a,b,c] = size(img);
X = reshape(img,[a*b c]); X=X'; X = single(X);
figure(1); clf; plot3( X(1,:), X(2,:), X(3,:), '*' );
title('three dimensions of the data');
...
plot( X(2,:), X(3,:), 'k*' );  
  str{1}=sprintf('data samples (p=%d, n=%d)',p,n);
hold all; 
plot( centers_Matlab(2,:), centers_Matlab(3,:), 'ws','markersize',...
    17,'markerfacecolor','b' ,'linewidth',2 );
  str{end+1}=sprintf('K-means (stat toolbox), %.2f seconds',time_Matlab);
plot( centers_faster(2,:), centers_faster(3,:), 'w^','markersize',...
    12,'markerfacecolor','g' ,'linewidth',2 );
  str{end+1}=sprintf('K-means (our toolbox), %.2f seconds',time_faster);
plot( centers_fastest(2,:), centers_fastest(3,:), ...
    'wd','markersize',12,'markerfacecolor','r','linewidth',2 );
  str{end+1}=sprintf('Sparsified K-means (%.0f%% sparse), %.2f seconds',...
      100*SparsityLevel,time_fastest);
...
plot3( X(1,:), X(2,:), X(3,:), 'k*' );  
  str{1}=sprintf('data samples (p=%d, n=%d)',p,n);
hold all; 
plot3( centers_Matlab(1,:), centers_Matlab(2,:), centers_Matlab(3,:), 'ws','markersize',...
    17,'markerfacecolor','b' ,'linewidth',4 );
  str{end+1}=sprintf('K-means (stat toolbox), %.2f seconds',time_Matlab);
plot3( centers_faster(1,:), centers_faster(2,:), centers_faster(3,:), 'w^','markersize',...
    12,'markerfacecolor','g' ,'linewidth',4 );
  str{end+1}=sprintf('K-means (our toolbox), %.2f seconds',time_faster);
plot3( centers_fastest(1,:), centers_fastest(2,:), centers_fastest(3,:), ...
    'wd','markersize',12,'markerfacecolor','r','linewidth',4 );
  str{end+1}=sprintf('Sparsified K-means (%.0f%% sparse), %.2f seconds',...
      100*SparsityLevel,time_fastest);
